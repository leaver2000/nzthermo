...
def sample_data():
    ...
    sounding_loc = Path("data/sounding.txt")
    ...
    specific_humidity = data[:, 3] * units(
        "kg/kg"
    )  # water vapor mass fraction (specific humidity), kg / kg
    u_wind = data[:, 4] * units("m/s")  # u wind, m / s
    v_wind = data[:, 5] * units("m/s")  # v wind, m / s

    dew_point_temperature = dewpoint_from_specific_humidity(
...
def test_end_to_end_ecape():
    ...
    height, pressure, temperature, specific_humidity, u_wind, v_wind, dew_point_temperature = (
        sample_data()
    )

    for cape_type in ["most_unstable", "surface_based", "mixed_layer"]:
        ecape = calc_ecape(
            height, pressure, temperature, specific_humidity, u_wind, v_wind, cape_type
        )
...
def test_undiluted_ecape():
    ...
    ecape = calc_ecape(
        height, pressure, temperature, specific_humidity, u_wind, v_wind, cape_type, undiluted_cape
    )
...
def test_calc_integral_arg():
    ...
    intarg_loc = Path("data/intarg.txt")
...
def test_calc_el_height():
    ...
    el_idx, el_z = calc_el_height(
        pressure, height, temperature, dew_point_temperature, most_unstable_parcel
    )
