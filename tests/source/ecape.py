...
def calc_sr_wind(
    pressure: PintList, u_wind: PintList, v_wind: PintList, height_msl: PintList
) -> pint.Quantity:
    """
    ...
    bunkers_right, _, _ = mpcalc.bunkers_storm_motion(
...
def calc_ecape(
    ...
    # calculate the buoyancy dilution potential (ncape)
    moist_static_energy_bar, moist_static_energy_star = calc_mse(
        pressure, height_msl, temperature, specific_humidity
    )
    integral_arg = calc_integral_arg(
        moist_static_energy_bar, moist_static_energy_star, temperature
    )
